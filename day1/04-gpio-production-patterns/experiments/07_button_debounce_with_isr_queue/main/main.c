...
{
    uint32_t gpio_num;
    TickType_t last_valid_press_time = 0;
    ...
    while (1)
    {
        if (xQueueReceive(gpio_evt_queue, &gpio_num, portMAX_DELAY))
        {
            if (gpio_num == BUTTON_GPIO)
            {
                ...
                TickType_t elapsed_time = now - last_valid_press_time;

                if (elapsed_time < pdMS_TO_TICKS(DEBOUNCE_TIME_MS))
                {
                    ESP_LOGW(TAG, "Bounce ignored");
                    continue;
                }
                ...
                {
                    ESP_LOGW(TAG, "False trigger ignored");
                    continue;
                }

                last_valid_press_time = xTaskGetTickCount();

                ESP_ERROR_CHECK(gpio_intr_disable(BUTTON_GPIO));

                ESP_LOGI(TAG, "Valid button press detected");
                ESP_LOGI(TAG, "Blinking LED %d times", LED_BLINK_COUNT);
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Button debounce experiment started");
